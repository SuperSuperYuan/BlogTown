...
| `aishelf.site.notes` | After `save_note` succeeds, trigger a **single-item** DB sync (off-thread) so the note becomes searchable promptly — mirrors the post-collection off-thread sync. Failure to sync is logged, never breaks saving the note (the note file is the source of truth; a rebuild recovers the index). | `db.sync` |
